...
Starts the full audio detection pipeline:
    ...
    → DetectionService → SQLite → ArtworkTrigger (background)

Usage:
    python run.py                        # static artwork
    python run.py --generated-artwork    # ComfyUI + Flux.1 (auto-generates)
    python run.py --device 5             # specific microphone
    python run.py --mock                 # MockBirdIdentifier (no model)
    python run.py --list-devices         # list microphones and exit

Requires separate terminals:
    python -m backend.api.main           # API server
    cd frontend && npm run dev           # dashboard
    cd ComfyUI_windows_portable && .\\run_nvidia_gpu.bat  # (if --generated-artwork)
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="BirdFrame live audio pipeline")
    ...
    parser.add_argument(
        ...
        help="Auto-generate artwork via ComfyUI + Flux.1 for new species",
    )
...
def build_identifier(args):
    ...
    return BirdNetIdentifier(
        latitude=lat, longitude=lon, week=week, min_confidence=0.05,
    )


def build_artwork_trigger(args):
    """Build ArtworkTrigger if --generated-artwork is set."""
    if not args.generated_artwork:
        return None

    from backend.detection.artwork_trigger import ArtworkTrigger
    return ArtworkTrigger(max_concurrent=1, enabled=True)


def build_service(identifier, artwork_trigger, args):
    ...
    return DetectionService(
        ...
        detector=detector,
        artwork_trigger=artwork_trigger,
...
def main():
    ...
    artwork_mode = "generated (auto)" if args.generated_artwork else "static"

    print("\n" + "=" * 60)
    print("  BirdFrame — Live Detection Pipeline")
    print("=" * 60)
    print(f"  Identifier : {'Mock' if args.mock else 'BirdNET'}")
    ...
    artwork_trigger = build_artwork_trigger(args)
    service = build_service(identifier, artwork_trigger, args)
    ...
    try:
        while not shutdown_requested:
            ...
            if now - last_stats_time >= 30:
                ...
                in_progress = stats.get("artwork_in_progress", [])
                completed = stats.get("artwork_completed", [])
                ...
                if in_progress:
                    print(f"  Generating artwork: {', '.join(in_progress)}")
                if completed:
                    print(f"  Artwork cached: {', '.join(completed)}")
                last_stats_time = now
        ...
        if args.generated_artwork:
            completed = stats.get("artwork_completed", [])
            print(f"  Artwork generated: {len(completed)} species")
            for s in completed:
                print(f"    ✓ {s}")
        print("=" * 60)
